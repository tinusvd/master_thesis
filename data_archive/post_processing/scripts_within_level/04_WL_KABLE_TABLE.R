...
table_function <- function(format_rules, output_dir = ".", par_conditions, rules_df) {
  ...
  for (table_idx in 1:n_tables) {
    ...
    for (cond_idx in 1:n_conditions) {
      ...
      table_of_interest <- paste0("summ_", table_id, "_wl")
      ...
      output_name <- paste0("table_", table_id, "_", gsub("condition_", "", par_con), "_wl")
...
# Example usage:
table_function(summary_rules, "./data_archive/figures_tables/tables/within_level", conditions_table, rules)
